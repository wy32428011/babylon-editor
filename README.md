...
预览工具栏提供“俯瞰地面”按钮，点击后会根据当前场景和 CAD 参考层在 XZ 地面的有效范围，自动把预览相机移动到地面上方并看向地面中心，方便检查平面布置、CAD 辅助图纸和模型落位关系。该操作只改变当前编辑器预览相机视角，不修改场景节点、保存结构或运行时导出。

预览工具栏提供“显示/隐藏网格”按钮，默认显示 XZ 水平面辅助网格，用于摆放模型时对齐位置和间距。网格支持 `4×4`、`8×8`、`16×16` 三种本机偏好值，对应显示为“小格 25 m / 12.5 m / 6.25 m”；切换后会立即刷新可视网格间距并写入本机 localStorage，下次打开编辑器保持上次选择。网格会跟随当前视口中心延伸，避免在远距离摆放时出现固定边界；网格线会保持柔和闪光，方便在大画布、暗色模型或远距离视角下识别当前位置。该网格是编辑器运行时辅助层，不保存到项目文件、不进入层级图，也不参与场景拾取、渲染导出或运行时逻辑；关闭按钮后只隐藏辅助线，不会修改场景节点。CAD 贴地参考层会渲染在地面上，工具栏提供独立的“显示/隐藏 CAD 参考图”开关，用于临时收起或恢复地面 CAD 辅助图纸；该开关只影响编辑器预览中的 CAD 派生参考对象，不删除 CAD 根节点和 metadata。预览网格使用更高的编辑器辅助渲染层，始终覆盖显示在 CAD 参考层之上。
...
拖入 GLB/GLTF/OBJ 等模型到场景画布时，编辑器会保留模型实例的真实世界尺寸和原始 `scaling`，不会为了适配编辑视口自动放大或缩小。若鼠标拖放位置命中了场景中的地面或已有模型，导入模型的底部中心会落到该命中点；若拖到空白区域，则使用当前鼠标射线与 XZ 水平面交点作为落点，使模型落在可视网格对应位置。拖放导入完成后会保持当前相机视角，只选中模型并显示 Gizmo，不再自动聚焦导致模型在屏幕上偏离刚才的鼠标指向位置。导入后仍可在右侧“变换”面板或画布 Gizmo 中继续精确调整位置、旋转和缩放。若模型在所有软件中都存在原点偏移、朝向错误或真实比例异常，仍建议回到 Blender 等建模工具修正源模型后重新导出。
...
预览工具栏提供“导入 CAD 图纸”入口，支持 `.dxf` 和 `.dwg`；也可以从系统文件管理器直接把 CAD 文件拖到预览画布，或在资产浏览器中双击/右键选择“导入 CAD 图纸”。所有入口都会先打开 CAD 导入配置弹窗，可设置文件、`cadId`、DWG 转换器路径、单位 `auto/mm/cm/m`、贴图最长边像素、透明度，以及是否生成矢量线图层。项目外 CAD 会复制到项目 `assets/cad-drawings/`，项目内 `.dxf` 会直接复用；`.dwg` 会先通过 Electron 主进程优先调用 ODA File Converter 转成 DXF，再进入同一解析链路。展会平面、厂房平面这类 DWG 默认建议按 `mm` 导入；例如 `2024年CEMAT展会方案2024.5.7(1).dwg` 这类 AC1024 展会图纸，导入弹窗默认使用 `mm`，如源图单位确认不同再改选 `cm` 或 `m` 后重新导入。

CAD 导入遵循 Babylon `1 世界单位 = 1 m`。单位选择为 `auto` 时读取 DXF 头部 `$INSUNITS`，当前自动识别 `mm/cm/m/inch/foot`；如果图纸没有可靠 `$INSUNITS`，会要求手动选择 `mm`、`cm` 或 `m`。坐标映射固定为 CAD `X -> Babylon X`、CAD `Y -> Babylon -Z`、CAD `Z -> Babylon Y`。导入后会生成唯一可见根节点 `CAD_GROUND_ROOT_{cadId}`、透明贴地参考图 `CAD_GROUND_MESH_{cadId}` 和按图层分组的 `CAD_LAYER_{layerName}`；Ground 使用透明底 DynamicTexture 绘制 CAD 线条，世界宽高按 CAD 有效包围盒和单位换算 1:1 创建，`isPickable = false`，矢量线图层使用 chunked `LineSystem` 避免大图纸创建过多 mesh。

新导入的 CAD 元数据写入根节点 `root.metadata.cad`，包含 `cadId/sourceFileName/sourcePath/projectSourcePath/projectRelativeSourcePath/importablePath/projectRelativeImportablePath/unit/unitScaleToMeter/bbox/origin/ground/axis/layers/inserts`。旧的 `metadata.cadDrawing` 仍保留兼容读取，但不再作为新结构主入口。保存项目时只保留 `CAD_GROUND_ROOT_{cadId}` 及其 metadata，`CAD_GROUND_MESH_{cadId}`、`CAD_LAYER_{layerName}` 和所有 LineSystem chunk 都标记为 CAD generated 派生对象，项目重开后优先按项目相对 DXF 路径重建，绝对路径只作为兜底，因此移动项目目录后不容易立刻失效。

CAD 参考层属于编辑器辅助对象，不属于业务运行场景。根节点和所有派生节点都会写入 `metadata.doNotSerialize = true`，Play、导出和 CLI pack 会排除这些节点；保存项目时为了后续继续辅助建模，只保留根节点 metadata，不保存透明贴地图、材质、贴图或矢量线派生节点。导入过程中顶部会显示分阶段进度条，编辑器 Console 会记录源文件、项目内路径、项目相对路径、DWG 转换命令、DXF 解析、贴地尺寸、图层数量和失败堆栈，便于排查无响应或转换失败。

#### DWG 转 DXF 插件下载与配置

当前 DWG 转 DXF 优先使用 ODA File Converter；LibreDWG/libdxfrw `dwg2dxf` 只作为显式配置或兜底候选，并且转换后的 DXF 必须通过结构校验后才会导入。ODA File Converter 通常更适合处理新版 DWG 和复杂图纸，但需要按 ODA 官方许可自行安装或配置，不建议未经许可直接随产品分发。可选下载入口：

- ODA File Converter 下载页：[https://www.opendesign.com/guestfiles/oda_file_converter](https://www.opendesign.com/guestfiles/oda_file_converter)
...
Windows 可在 CAD 导入配置弹窗中直接填写或选择 `ODAFileConverter.exe` 路径。也可以设置 `ODA_FILE_CONVERTER` 指向可执行文件，或设置 `BABYLONJS_EDITOR_ODA_CONVERTER_DIR` 指向安装目录；开发环境可把转换器放在仓库根目录的 `bin/cad/`，打包后的 Electron 应用可放在安装目录的 `resources/bin/cad/` 或 `resources/app.asar.unpacked/bin/cad/`。自动探测优先级为：弹窗中的用户路径、`ODA_FILE_CONVERTER`、`BABYLONJS_EDITOR_ODA_CONVERTER_DIR`、编辑器随带 `bin/cad`、PATH 中的 `ODAFileConverter`、`LIBREDWG_DWG2DXF`、编辑器随带 `dwg2dxf`、PATH 中的 `dwg2dxf`。ODA 调用格式为 `ODAFileConverter <输入目录> <输出目录> ACAD2018 DXF 0 1 <文件名.dwg>`；LibreDWG 兜底调用格式为 `dwg2dxf -y -o <输出DXF> <输入DWG>`。

配置完成后重新导入 `.dwg` 即可。转换后的 DXF 会先做结构校验：优先使用 `ENTITIES` 模型空间实体段；如果转换器只输出 `BLOCKS`，但其中包含可绘制的 `LINE`、`POLYLINE`、`LWPOLYLINE`、`CIRCLE`、`ARC`、`SPLINE` 或 `INSERT`，导入器会记录警告并按 `BLOCKS/INSERT` 递归展开。部分 `dwg2dxf` 输出缺少标准 `0/EOF` 结束标记时会在解析前自动补齐；如果 `ENTITIES` 和 `BLOCKS` 都没有可绘制几何，仍会拒绝导入。通过校验后，DXF 会交给 `dxf-parser` 读取并展开 `INSERT` 引用，支持块基点、插入点、旋转、X/Y 缩放、行列阵列和嵌套块递归保护，块内 `0/ByBlock` 图层会继承插入图层。`INSERT` 仍会写入 metadata 便于排查；暂不支持 HATCH、复杂 MTEXT、XREF、3D CAD 实体、瓦片切图或自动墙体拉伸。若导入失败，编辑器 Console 会给出缺少转换器、无效 DWG、空 DXF、转换后 DXF 结构异常、未知 auto 单位或空包围盒等明确中文错误。
...
- 保存 `.params.ts` 并修改装饰器字段默认值时，未在 Inspector 中手动覆盖过的字段会跟随脚本新默认值实时同步；已经手动调过的场景参数会记录为 `overridden` 并保持 Inspector 值，不会被脚本保存动作覆盖。
