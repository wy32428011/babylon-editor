...
export class Dashboard extends Component<IDashboardProps, IDashboardState> {
					...
					<div className="flex flex-col gap-4 flex-[0_0_auto]">
						...
						<Fade delay={500}>
							<div className="flex justify-between items-center">
								<div className="text-3xl font-semibold">项目</div>
		...
		if (!nodeJSAvailable) {
			await showAlert(
				...
				<div className="flex flex-col">
					<div>系统中未找到 Node.js。</div>
					<div>
						构建和运行项目需要 Node.js。可以通过{" "}
	...
	private _handleImportProject(): unknown {
		...
		if (exists) {
			return showAlert("项目已存在", "要导入的项目已存在于仪表盘中。");
		}
	...
	private async _tryRemoveProjectFromLocalStorage(project: ProjectType): Promise<void> {
		const confirm = await showConfirm("移除项目", "确定要移除此项目吗？");
		if (!confirm) {
			return;
		}
