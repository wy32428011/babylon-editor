...
export class CustomMetadataInspector extends Component<ICustomMetadataInspectorProps> {
	public render(): ReactNode {
		...
		return (
			<EditorInspectorSectionField title="元数据">
				{!keys.length && <div className="text-center text-white/50 py-2">No metadata found. Click "添加" to create a new key-value pair.</div>}
	...
	private async _handleAddKey(): Promise<unknown> {
		const key = await showPrompt("添加自定义元数据", "Enter the key name for the new metadata:");
