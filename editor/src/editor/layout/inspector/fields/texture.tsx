...
export class EditorInspectorTextureField extends Component<IEditorInspectorTextureFieldProps, IEditorInspectorTextureFieldState> {
	...
	private _copyMissingTexture(textureUrl: string): void {
		...
		toast.info("纹理名称已复制到剪贴板", { duration: 3000 });
	}
	...
	private _getCubeTextureInspector(): ReactNode {
		...
		return (
			<div className="flex flex-col gap-2 h-full">
				...
				<EditorInspectorSectionField title="坐标">
					<EditorInspectorNumberField
						label="Index"
						object={texture}
					...
					<EditorInspectorListField
						...
						items={[
							...
							{ text: "天空盒", value: Texture.SKYBOX_MODE },
	...
	private _getTextureInspector(): ReactNode {
		...
		return (
			<div className="flex flex-col gap-2 h-full">
				...
				<EditorInspectorSectionField title="偏移">
					<EditorInspectorNumberField
						noUndoRedo={this.props.noUndoRedo}
						label="U Offset"
						object={texture}
				...
				<EditorInspectorSectionField title="采样">
					<EditorInspectorListField
						noUndoRedo={this.props.noUndoRedo}
						label="模式"
						object={o}
				...
				<EditorInspectorSectionField title="重复">
					<EditorInspectorListField
						noUndoRedo={this.props.noUndoRedo}
						label="重复 U"
						object={texture}
						...
						onChange={() => this.props.onChange?.(texture)}
						items={[
							{ text: "重复", value: Texture.WRAP_ADDRESSMODE },
							{ text: "钳制", value: Texture.CLAMP_ADDRESSMODE },
							{ text: "镜像", value: Texture.MIRROR_ADDRESSMODE },
						]}
					/>
					<EditorInspectorListField
						noUndoRedo={this.props.noUndoRedo}
	...
	private async _computeTemporaryPreview(): Promise<void> {
		...
		const buffer = (await sharp(path).resize(128, 128).toBuffer()) as Buffer;
	...
	private async _computeTemporaryPreviewFromDataString(dataString: string): Promise<void> {
		...
		if (dataType === "base64") {
			const buffer = (await sharp(Buffer.from(byteString, "base64")).resize(128, 128).toBuffer()) as Buffer;
