...
export class EditorSkeletonInspector extends Component<IEditorInspectorImplementationProps<Skeleton>> {
	...
	public render(): ReactNode {
		return (
			<>
				<EditorInspectorSectionField title="骨骼">
				...
				<EditorInspectorSectionField title="动画范围">
					{this.props.object
						.getAnimationRanges()
						.filter((range) => range)
						.map((range, index) => (
									...
									<Button
										...
										onClick={() => {
											try {
												...
												toast.success("动画范围名称已复制到剪贴板");
											} catch (e) {
												toast.error("复制动画范围名称失败");
											}
										}}
									>
										<FaCopy />
					...
					<Button
						...
						onClick={async () => {
							const name = await showPrompt("添加动画范围", "Enter the name of the new animation range");
							if (name) {
				...
				<EditorInspectorSectionField title="绑定网格">
