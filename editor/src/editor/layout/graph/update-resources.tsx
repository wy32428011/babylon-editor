...
export async function showUpdateResourcesFromAsset(editor: Editor, node: Node) {
	const filename = openSingleFileDialog({
		...
		filters: [
			...
			{ name: "Babylon.js 场景文件", extensions: ["babylon"] },
		],
			...
			<AlertDialog open>
				<AlertDialogContent className="min-w-[50vw] h-fit max-h-[50vh] overflow-y-auto transition-all duration-300 ease-in-out">
					<AlertDialogHeader className="w-full">
						<AlertDialogTitle>从资源更新引用</AlertDialogTitle>
					...
					<AlertDialogFooter>
						<AlertDialogCancel onClick={() => this.props.onClose()}>取消</AlertDialogCancel>
			...
			<Table>
				...
				<TableHeader>
					<TableRow>
						...
						<TableHead className="w-full">目标网格</TableHead>
					</TableRow>
				</TableHeader>
				<TableBody>
					{components.map((c) => (
			...
			<Table>
				...
				<TableHeader>
					<TableRow>
						...
						<TableHead className="w-full">资源根节点</TableHead>
					</TableRow>
				</TableHeader>
				<TableBody>
					{components.map((c) => (
