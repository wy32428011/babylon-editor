...
export class MarketplaceSettingsDialog extends Component<IMarketplaceSettingsDialogProps, IMarketplaceSettingsDialogState> {
		...
		return (
			<AlertDialog open={this.props.open}>
				<AlertDialogContent className="sm:max-w-[600px]">
					<AlertDialogHeader>
						<AlertDialogTitle className="text-2xl font-bold">资源市场设置</AlertDialogTitle>
					</AlertDialogHeader>

					<div className="flex flex-col gap-6 py-4 max-h-[60vh] overflow-y-auto pr-2 w-full">
						{/* Global 资源市场设置 */}
						<div className="flex flex-col gap-4">
							<h3 className="text-lg font-semibold flex items-center gap-2">全局设置</h3>
					...
					<AlertDialogFooter>
						<AlertDialogCancel onClick={() => this.props.onClose()}>关闭</AlertDialogCancel>
					</AlertDialogFooter>
				</AlertDialogContent>
			</AlertDialog>
		);
	...
	private _handleBrowseDownloadPath(): void {
		const path = ipcRenderer.sendSync("editor:open-single-folder-dialog", "Select 下载路径");
		if (path) {
