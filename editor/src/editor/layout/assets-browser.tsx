...
export class EditorAssetsBrowser extends Component<IEditorAssetsBrowserProps, IEditorAssetsBrowserState> {
	...
	private async _refreshFilesTreeNodes(path: string): Promise<void> {
		...
		this.setState({
			filesTreeNodes: [
				{
					label: <div className="ml-2 p-1">项目</div>,
					id: "/",
					nodeData: "/",
					isExpanded: true,
			...
			<div className="flex flex-col w-full h-full">
				<div className="flex gap-2 justify-between px-2 w-full h-10 min-h-10 bg-primary-foreground">
					<div className="flex gap-2 items-center h-full">
						...
						<Button variant="ghost" className="w-8 h-8 p-0.5" title="进入上一个目录">
							<FaArrowRight className="w-4 h-4" />
						</Button>
						<Button variant="ghost" className="w-8 h-8 p-0.5" disabled={!this.state.browsedPath} title="刷新" onClick={() => this.refresh()}>
							<MdOutlineRefresh className="w-5 h-5" />
						</Button>

						<Button variant="ghost" className="gap-2 w-24 h-8 p-0.5" title="将现有资源导入当前目录" onClick={() => this._handleImportFiles()}>
							<IoArrowDownCircleOutline className="w-5 h-5" /> 导入
						</Button>
					</div>
					...
					<div className="flex gap-2 items-center">
						...
						<DropdownMenu onOpenChange={(o) => o && this.forceUpdate()}>
							...
							<DropdownMenuContent>
								...
								>
									{this.state.showGeneratedFiles ? <IoCheckmark /> : ""} 显示生成文件
								</DropdownMenuItem>
		...
		return (
			<>
				...
				{this.props.editor.state.enableExperimentalFeatures && (
					<>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddCinematic()}>过场动画</ContextMenuItem>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddNavmesh()}>Navmesh</ContextMenuItem>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddRagdoll()}>布娃娃</ContextMenuItem>
					</>
				)}
				...
				<ContextMenuSeparator />
				<ContextMenuItem onClick={() => this._handleAddScript("params")}>模型参数脚本</ContextMenuItem>
				<ContextMenuItem onClick={() => this._handleAddScript("animation")}>模型动画驱动脚本</ContextMenuItem>
			</>
		);
	}
	...
	private async _handleDroppedNodesFromGraph(nodeIds: string[]): Promise<void> {
		...
		if (!this.isAssetsFolder()) {
			showAlert("警告", <div>只能将节点导出到根 assets 文件夹内。</div>, true);
			return;
		}
		...
		for (const nodeId of nodeIds) {
			...
			if (node) {
				...
				if (await pathExists(scenePath)) {
					const overwrite = await showConfirm("覆盖场景？", `A scene named "${node.name}" already exists at this location. Do you want to overwrite it?`);
					if (!overwrite) {
						continue;
					}
				}
	...
	private async _handleAddNodeParticleSystem(): Promise<void> {
		...
		const npe = new NodeParticleSystemSet("New 节点粒子系统 Set");
		npe.setToDefault();
	...
	/**
	 * 在当前浏览目录中创建脚本模板文件。
	 * @param type 定义要创建的普通脚本或模型外挂脚本模板类型。
	 */
	private async _handleAddScript(type: "class" | "function" | "params" | "animation"): Promise<void> {
		if (!this.state.browsedPath) {
			return;
		}
		...
		const scriptTemplates = {
			class: {
				url: "assets/class-based-script.ts",
				name: "new-script",
			},
			function: {
				url: "assets/function-based-script.ts",
				name: "new-script",
			},
			params: {
				url: "assets/model-params-script.ts",
				name: "model.params",
			},
			animation: {
				url: "assets/model-animation-driver-script.ts",
				name: "model.anim.default",
			},
		};

		const template = scriptTemplates[type];
		const content = await fetch(template.url).then((r) => r.text());

		const name = await findAvailableFilename(this.state.browsedPath, template.name, ".ts");
	...
	private async _handleLoadScene(absolutePath: string, newlyCreated?: boolean): Promise<void> {
		...
		if (!newlyCreated) {
			const accept = await showConfirm("确定吗？", "这将关闭当前场景并打开选中的场景。");
			if (!accept) {
				return;
			}
		}

		const acceptSave = await showConfirm("保存当前场景？", "Do you want to save the current scene before opening the new one?", {
			confirmText: "保存",
			cancelText: "不保存",
		});
