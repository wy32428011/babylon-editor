...
import { loadScene, scriptsDictionary, scriptAssetsCache, dispatchMqttValueToObject, _applyScriptsForObject, _removeRegisteredScriptInstance, _preloadScriptsAssets } from "babylonjs-editor-tools";
{{imports}}
...
export { loadScene, scriptsDictionary, scriptAssetsCache, dispatchMqttValueToObject, _applyScriptsForObject, _removeRegisteredScriptInstance, _preloadScriptsAssets };
`;
 ...
 * 根据脚本存储根目录返回脚本绝对路径。
 */
const getScriptAbsolutePath = (projectDir: string, script: any): string => {
	return script.root === "project" ? join(projectDir, script.key) : join(projectDir, "src", script.key);
};

/**
 * 返回生成的 src/scripts.ts 中使用的模块导入路径。
 */
const getScriptImportPathFromScriptsFile = (script: any): string => {
	const keyWithoutExtension = script.key.replace(extname(script.key), "");
...
export async function createScriptsFile(projectDir: string): Promise<void> {
		...
		configuration.metadata.scripts?.forEach((script) => {
			promises.push(
				new Promise<void>(async (resolve) => {
					const path = getScriptAbsolutePath(projectDir, script);
					if (!(await fs.pathExists(path))) {
						return resolve();
					}

					scriptsMap[script.key] = getScriptImportPathFromScriptsFile(script);
	...
	Object.keys(scriptsMap).forEach((key) => {
		...
		importStatements.push(`import * as ${scriptName} from "${scriptsMap[key]}";`);
	});
